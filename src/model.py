...
from calculation_helper import calculate_shell,s_shell_init
...
class LabelPropagator:
    ...
    def DNSD(self,graph):
        ...
        for i in graph.nodes:
            s_init = s_shell_init(graph, int(i))
            dic_valuf_shell[i] = s_init
        copy_graph_delet = graph.copy()
        while len(nx.nodes(copy_graph_delet)) != 0:
            min_shell = min(dic_valuf_shell.values())
            res = [key for key in dic_valuf_shell if dic_valuf_shell[key] == min_shell]
            for h in res:
                influence[h] = min_shell
                copy_graph_delet.remove_node(int(h))
            for n in nx.nodes(copy_graph_delet):
                value = calculate_shell(graph, n, res, dic_valuf_shell)
                dic_valuf_shell[int(n)] = value
            for h in res:
                del dic_valuf_shell[h]
        return influence
